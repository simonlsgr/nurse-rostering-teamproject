...
def test_minimum_consecutive_shifts_feasible_table():
    shifts = [
        ...
        Shift(
            demand=1,
            start_time=datetime.datetime(2018, 1, 5, 8, 0),
            end_time=datetime.datetime(2018, 1, 5, 16, 0),
            name="Morning Shift",
            type="A",
        ),
        Shift(
    ...
    with AssertModelFeasible() as model:
        ...
        MinimumConsecutiveShiftsModuleTable().build(instance, model, nv)

        nv.fix(nurse1.uid, shifts[0].uid, False)
        ...
        nv.fix(nurse1.uid, shifts[8].uid, False)
        nv.fix(nurse1.uid, shifts[9].uid, False)
