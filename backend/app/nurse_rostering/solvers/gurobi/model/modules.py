        ...
        for nv in nurse_shift_vars:
            for date, shift_uids in shifts_by_date.items():
                ...
                if len(vars_for_date) > 1:
                    model.addConstr(sum(vars_for_date) <= 1)
        return 0
                ...
                for day_shift in day_shifts:
                    ...
                    for following_shift in following_day_shifts:
                        ...
                        if _type is None:
                            continue
                        if _type in types:
                            model.addConstr(nv.is_assigned_to(day_shift) + nv.is_assigned_to(following_shift) <= 1)
    ...
    def build(self, instance, model, nurse_shift_vars):
        ...
        for nv in nurse_shift_vars:
            for type, nb_types in nv.nurse.maximum_number_of_shifts_per_type.items():
                model.addConstr(
                    gp.quicksum(nv.is_assigned_to(shift.uid) for shift in instance.shifts if shift.type == type) <= nb_types
                )
    ...
    def build(self, instance, model, nurse_shift_vars):
        ...
        for nv in nurse_shift_vars:
            ...
            for d in range(len(all_dates) - max_shifts):
                ...
                model.addConstr(gp.quicksum(nv.is_assigned_to(shift) for shift in shifts_in_range) <= max_shifts)
        return 0
